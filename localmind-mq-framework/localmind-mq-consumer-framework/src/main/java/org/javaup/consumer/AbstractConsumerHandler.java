...
public abstract class AbstractConsumerHandler<T> {
    ...
    protected Boolean beforeConsume(MessageExtend<T> message) {
        log.info("RocketMQ message before consume success, uuid={}, key={}", message.getUuid(), message.getKey());
        return true;
    }
    ...
    protected void afterConsumeSuccess(MessageExtend<T> message) {
        log.info("RocketMQ message consume success, uuid={}, key={}", message.getUuid(), message.getKey());
    }
    
    protected void afterConsumeFailure(MessageExtend<T> message, Throwable throwable) {
        log.error("RocketMQ message consume failed, uuid={}, key={}, messageBody={}",
